...
def get_parser():
    ...
    parser.add_argument('--bandpassLow', help='file to write full probabilty data to', default=34000)
    parser.add_argument('--bandpassHigh', help='file to write full probabilty data to', default=68000)
	# fmt: on
...
for filename in glob.glob(os.path.join(args.datadir, '*.wav')):
	...
	data = fir_band_pass(data, rate, args.bandpassLow, args.bandpassHigh, 461, 461, np.int16)
    #this might improve some results by amplifying USVs, but testing was pretty poor
	#data = data*2
